...
export const BottomNav = ({ currentPath }: BottomNavProps) => {
  const navItems = [
    { path: "/", icon: Circle, label: "Home", color: "#A9A9A9" },
  ...
  return (
    <nav className="fixed bottom-0 left-0 right-0 bg-card/80 backdrop-blur-md border-t-2 border-border shadow-[0px_-4px_0px_hsl(var(--border))]">
      <div className="max-w-6xl mx-auto flex items-center justify-around px-2 py-4 safe-area-bottom">
        {navItems.map(({ path, icon: Icon, label, color }) => {
          ...
          return (
            <Link
              key={path}
              to={path}
              className="flex flex-col items-center gap-1.5 transition-all hover-scale"
              style={{
                transform: active ? 'scale(1.1)' : 'scale(1)',
                filter: active ? `drop-shadow(0 0 12px ${color}40)` : 'none'
              }}
            >
              <Icon 
